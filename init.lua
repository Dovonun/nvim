...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      -- vim.keymap.set('n', '<leader>db', actions.delete_buffer(1), { desc = '[d]elete [b]uffer' })
    end,
  },
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        ...
        local lsp_format_ops
        if disable_filetypes[vim.bo[bufnr].filetype] then
          lsp_format_ops = 'never'
        else
          lsp_format_ops = 'fallback'
        end
        return {
          timeout_ms = 500,
          lsp_fallback = lsp_format_ops,
        }
      end,
      formatters_by_ft = {
        lua = { 'stylua' },
...
vim.keymap.set('n', '<leader>R', [[:<C-u>execute 'let @" = substitute(@", ''\\n'', ''\\\\n'', ''g'')' | %s/<C-r>"//<left>]])
